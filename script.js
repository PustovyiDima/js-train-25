...
async function getData() {
   try {
      await fetchFakeData().then((res) => console.log("Завдання: 1:", res));
   } catch (error) {
      console.error(error);
   }
}
...
async function logRandomNumberAfterSeconds(seconds) {
   try {
      ...
      console.log("Завдання: 2:", randomNumber);
   } catch (error) {
      console.error(error);
   }
}
...
async function getDataFromUrl(url) {
   try {
      const result = await fetch(url, {
         method: "GET",
      }); //.then((result) => console.log("Завдання: 3 :", result));
      if (!result.ok) {
         throw new Error("its not ok");
      }
      const dataFromServer = await result.json();
      console.log("Завдання: 3 :", dataFromServer);
...
async function postDataWithAuth(url, data, authToken) {
   try {
      ...
      const dataFromServer = await result.json();
      console.log("Завдання: 4 :", dataFromServer);
   } catch (error) {
      console.error(error);
   }
}
...
async function displayData() {
   ...
   const gatherDataFunct = gatherData();

   console.log((await gatherDataFunct.next()).value);
   console.log((await gatherDataFunct.next()).value);
   console.log((await gatherDataFunct.next()).value);
}
